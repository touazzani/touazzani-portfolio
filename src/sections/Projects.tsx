...
const Projects = () => {
  ...
  useEffect(() => {
    ...
    typeTimeline.current
      ...
      .to(cursorEl, { autoAlpha: 0, repeat: -1, yoyo: true, duration: 0.5, ease: 'steps(1)' })
      
      .to(sloganEl, {
        text: sloganText,
        duration: sloganText.length * 0.05, // Vitesse de frappe
        ease: 'none',
      }, 0) // Le '0' démarre l'animation en même temps
      ...
      <div className="mx-auto aspect-video relative overflow-hidden shadow-2xl">
        ...
        >
          ...
          {projectList.map((project: Project, index: number) => (
            ...
            <div key={project.id} className="w-full flex-shrink-0 h-full relative">
              <img
                src={cozyTest} // Utilise l'image de l'objet projet
              ...
              <div className="container absolute bottom-0 md:py-16 text-white flex flex-col md:gap-4">
                  ...
                  <div className="flex gap-4">
                    <button className='bg-black text-whtie px-5 py-2 font-semibold rounded-md w-fit mt-3 cursor-pointer' onClick={() => setSelectedProject(project)}>
                      Voir plus
                    </button>
                    {project.link &&                   
                    <a
        ...
        <Info project={selectedProject} onClose={() => setSelectedProject(null)}/>
      </div>
